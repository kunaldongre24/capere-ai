...
export default async function SeoPage({ searchParams }: { searchParams: Promise<{ view?: string }> }) {
  ...
  else content = <><div className="grid grid-4"><Stat label="Organic clicks" value={clicks ?? '—'} detail={clicks ? 'Search Console synced' : 'Awaiting metric refresh'}/><Stat label="Impressions" value={impressions ?? '—'} detail={impressions ? 'Search Console synced' : 'Awaiting metric refresh'}/><Stat label="Average position" value={position ?? '—'} detail={position ? 'Search Console synced' : 'Awaiting metric refresh'}/><Stat label="Technical health" value="—" detail="Run a DataForSEO audit"/></div><div className="grid grid-3" style={{ marginTop: 12 }}><EmptyState title="Technical SEO" body="Run a DataForSEO technical audit to populate health scores and prioritized issues." badge="Audit required"/><EmptyState title="Keywords & competitors" body="Search Console is connected and collecting query data." badge="GSC connected"/><EmptyState title="Google Business Profile" body="Connect a GBP location to monitor ratings, reviews, calls, and directions." badge="GBP not connected"/></div></>;
